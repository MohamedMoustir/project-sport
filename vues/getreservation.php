...
if (isset($_GET['delete'])) {
  ...
  $query->execute();
  if ($query) {
    header("Location: ../vues/getreservation.php?delete =" . $id);
    exit;
  }
}
              ...
              <?php if ($list->statuss): ?>
                <?php if ($list->statuss == 'confirm'):
                  ...
                  <button
                    ...
                    class="inline-block <?= $hidden ?>  rounded bg-red-700 px-6 pb-2 pt-2.5 text-xs font-medium uppercase leading-normal text-white shadow-danger-3 transition duration-150 ease-in-out hover:bg-danger-accent-300 hover:shadow-danger-2 focus:bg-danger-accent-300 focus:shadow-danger-2 focus:outline-none focus:ring-0 active:bg-danger-600 active:shadow-danger-2 motion-reduce:transition-none dark:shadow-black/30 dark:hover:shadow-dark-strong dark:focus:shadow-dark-strong dark:active:shadow-dark-strong">
                    <a href="../vues/getreservation.php?delete =<?= $list->idResvations ?>"> delete </a>
                  </button>
