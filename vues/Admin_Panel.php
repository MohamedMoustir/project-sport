...
if (isset($_SESSION['email'])) {
    $email = $_SESSION['email'];
    $emai = $_SESSION['email'];
    $Countreservation = $pdo->prepare("SELECT COUNT(*) AS row_coun FROM resvations join users on  users.id = resvations.idAvocat  where users.email = :email and statuss = 'refuser' ");
    $Countreservation->bindParam(':email', $email, PDO::PARAM_STR);
    $Countreservation->execute();
    $rowss = $Countreservation->fetch(PDO::FETCH_ASSOC);
}
